...
use crate::storage::{
    BlockingNorStorageBusDriver, NorStorageBusError, NorStorageCmd, NorStorageCmdMode, NorStorageCmdType,
};
...
impl NorStorageBusError for FlexSpiError {
    fn decode_bus_error(&self) {
        self.describe();
    }
}

impl crate::storage::BlockingNorStorageBusDriver for FlexspiNorStorageBus<Blocking> {
    fn execute_cmd(
        &mut self,
        cmd: NorStorageCmd,
        ...
        self.program_lut(&cmd, 5);

        self.setup_ip_transfer(5, cmd.addr, cmd.data_bytes);
        self.execute_ip_cmd();
        ...
        if let Some(data_cmd) = cmd.cmdtype {
            ...
            match data_cmd {
                NorStorageCmdType::Read => {
                    ...
                    self.read_cmd_data(buffer.len(), buffer);
                }
...
impl<M: Mode> FlexspiNorStorageBus<M> {
    ...
    fn program_cmd_instruction(&self, cmd: &NorStorageCmd, instruction_counter: &mut usize) {
        ...
        self.info
            .regs
            .lut(seq_id)
            .modify(|_, w| unsafe { w.opcode0().bits(cmd_mode as u8) });
        self.info
            .regs
            .lut(seq_id)
            .modify(|_, w| unsafe { w.num_pads0().bits(cmd.bus_width) });
        self.info
            .regs
            .lut(seq_id)
            .modify(|_, w| unsafe { w.operand0().bits(cmd.cmd_lb) });

        if cmd.cmd_ub.is_some() {
            self.info
                .regs
                .lut(seq_id)
                .modify(|_, w| unsafe { w.opcode1().bits(cmd_mode as u8) });
            self.info
                .regs
                .lut(seq_id)
                .modify(|_, w| unsafe { w.num_pads1().bits(cmd.bus_width) });
            self.info
                .regs
                .lut(seq_id)
                .modify(|_, w| unsafe { w.operand1().bits(cmd.cmd_ub.unwrap()) });
        }
        seq_id += 1;
    ...
    fn program_addr_instruction(&self, cmd: &NorStorageCmd, instruction_counter: &mut usize) {
        ...
        if seq_id % 2 == 0 {
            ...
            self.info
                .regs
                .lut(seq_id)
                .write(|w| unsafe { w.num_pads0().bits(cmd.bus_width) });
            self.info
                .regs
                .lut(seq_id)
                .write(|w| unsafe { w.operand0().bits(cmd.addr_width.unwrap() * 8) });
        } else {
            self.info
                .regs
                .lut(seq_id)
            ...
            self.info
                .regs
                .lut(seq_id)
                .write(|w| unsafe { w.num_pads1().bits(cmd.bus_width) });
            self.info
                .regs
                .lut(seq_id)
                .write(|w| unsafe { w.operand1().bits(cmd.addr_width.unwrap() * 8) });
        }
        seq_id += 1;
        *instruction_counter = seq_id;
    }
...
impl FlexspiNorStorageBus<Blocking> {
    fn setup_ip_transfer(&mut self, seq_id: u8, addr: Option<u32>, size: Option<u8>) {
        match addr {
            Some(addr) => {
            ...
            None => {
                self.info.regs.ipcr0().write(|w| unsafe { w.sfar().bits(0) });
            }
        }
