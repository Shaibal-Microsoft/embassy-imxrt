...
use storage_bus_service::storage_bus::{
    ...
    NorStorageCmdMode, NorStorageCmdType,
};
...
impl<M: Mode> FlexspiNorStorageBus<M> {
    ...
    fn program_dummy_instruction(&self, cmd: &NorStorageCmd, cookie: &mut LutInstrCookie) {
        ...
        if cookie.instr_num == 0 {
            self.write_even_instr(seq_id, cmd_mode, cmd.dummy.unwrap(), bus_width);
        } else {
            self.write_odd_instr(seq_id, cmd_mode, cmd.dummy.unwrap(), bus_width);
        }
        cookie.next_instruction();
    }

    fn program_read_data_instruction(&self, cmd: &NorStorageCmd, cookie: &mut LutInstrCookie) {
        let seq_id = cookie.seq_num as usize;
        ...
        if cookie.instr_num == 0 {
            self.write_even_instr(seq_id, cmd_mode, cmd.data_bytes.unwrap(), bus_width);
        } else {
            self.write_odd_instr(seq_id, cmd_mode, cmd.data_bytes.unwrap(), bus_width);
        }
        cookie.next_instruction();
    }

    fn program_write_data_instruction(&self, cmd: &NorStorageCmd, cookie: &mut LutInstrCookie) {
        let seq_id = cookie.seq_num as usize;
    ...
    fn program_lut(&self, cmd: &NorStorageCmd, seq_id: usize) {
        ...
        if cmd.data_bytes.is_some() {
            if let Some(transfertype) = cmd.cmdtype {
                match transfertype {
                    NorStorageCmdType::Read => {
                        self.program_read_data_instruction(cmd, &mut cookie);
                    }
                    NorStorageCmdType::Write => {
                        self.program_write_data_instruction(cmd, &mut cookie);
                    }
                }
            }
        }
