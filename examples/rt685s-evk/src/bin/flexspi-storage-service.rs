...
use embassy_imxrt::storage::{BlockingNorStorageBusDriver, NorStorageCmd, NorStorageCmdMode, NorStorageCmdType};
    ...
    fn read(&mut self, offset: u32, bytes: &mut [u8]) -> Result<(), Self::Error> {
        ...
        if bytes.len() <= Self::READ_SIZE {
            ...
            let read_cmd = NorStorageCmd {
                ...
                data_bytes: Some(bytes.len() as u8),
            };

            let _ = self.storagebusdriver.execute_cmd(read_cmd, Some(&mut buffer), None);
        } else {
            loop {
                ...
                let read_cmd = NorStorageCmd {
                    ...
                    dummy: Some(20),
                    cmdtype: Some(NorStorageCmdType::Read),
                    data_bytes: Some(Self::READ_SIZE as u8),
                };
            ...
            if bytes.len() - read_start_ptr > 0 {
                let read_cmd = NorStorageCmd {
                    ...
                    dummy: Some(20),
                    cmdtype: Some(NorStorageCmdType::Read),
                    data_bytes: Some((bytes.len() - read_start_ptr) as u8),
                };
...
impl<T: BlockingNorStorageBusDriver> BlockingNorFlash for MacronixDeviceDriver<T, Blocking> {
    ...
    fn erase(&mut self, mut from: u32, to: u32) -> Result<(), Self::Error> {
        ...
        loop {
            ...
            let _ = self.storagebusdriver.execute_cmd(
                NorStorageCmd {
                    cmd_lb: 0x21,
                    cmd_ub: Some(0xDE),
    ...
    fn write(&mut self, offset: u32, bytes: &[u8]) -> Result<(), Self::Error> {
        ...
        let _ = bus_ref.execute_cmd(write_enable_cmd, None, None);
        ...
        let _ = bus_ref.execute_cmd(read_status_cmd, Some(&mut status), None);
        ...
        if bytes.len() <= Self::WRITE_SIZE {
            ...
            loop {
                ...
                let write_cmd = NorStorageCmd {
                    ...
                    dummy: None,
                    cmdtype: None,
                    data_bytes: Some(Self::WRITE_SIZE as u8),
                };
                let _ = bus_ref.execute_cmd(write_cmd, None, Some(&bytes[write_start_ptr..write_end_ptr]));
            ...
            if bytes.len() - write_start_ptr > 0 {
                let write_cmd = NorStorageCmd {
                    ...
                    dummy: None,
                    cmdtype: None,
                    data_bytes: Some((bytes.len() - write_start_ptr) as u8),
                };
                let _ = bus_ref.execute_cmd(write_cmd, None, Some(&bytes[write_start_ptr..bytes.len()]));
            }
        }
